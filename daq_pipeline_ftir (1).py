...
TBN/Absorbance estimation — FTIR Multiple Linear Regression (replaces cubic-spline interpolation):
  Instead of interpolating TBN from sparse lab measurements over engine-hours, TBN is now
  predicted from FTIR absorbance averages across 5 spectral bands using the multiple linear
  regression model from Wolak et al. (Energies 2022, 15, 2809) — Model C (R²=0.982, RMSE=0.405):

    TBN = 9.682 + 316.270·Abs(<1000) - 10.326·Abs(1000–1450) - 133.000·Abs(1475–1800)
               - 36.081·Abs(1800–2830) + 3.841·Abs(2975–4000)

  Additional co-variates (temperature, turbidity, fuel dilution) are supported via an
  extended MLR that appends optional correction terms when the sensor provides them.
  Absorbance can itself be estimated from wavenumber, temperature, turbidity, and fuel
  dilution using a secondary linear model fit offline from lab calibration data.

  The FTIRModel class handles both directions:
    • absorbance_from_conditions(wavenumber, temp, turbidity, fuel_dilution) → float
    • tbn_from_absorbances(abs_bands: dict) → float
    • estimate(ftir_input: FTIRInput) → {"tbn": float, "absorbances": dict}

  Fallback: if FTIR inputs are unavailable the model degrades gracefully to the
  paper bootstrap knots (same spline path as before), keeping the pipeline live.

Architecture:
  Sensor → StreamReader → RingBuffer → [Rust subprocess / C-kernel fallback] → Dashboard
                                              ↑
                                     (centroids.bin — written offline)
                                      ...
                                      OfflineTrainer (idle / scheduled / CLI)
                                        ...
                                        4. Refit FTIR model coefficients from lab data
                                        5. Atomic binary export → Rust reloads

KMeans inference (Python fallback path):
  When the Rust binary is unavailable the fallback now uses a JIT-compiled C shared
  library (gcc -O3 -march=native) for the L1/Manhattan nearest-centroid step.
  Benchmark: ~1.65 µs/call vs ~4.3 µs for scipy cdist — 2.6× faster, sub-2µs latency
  even on Raspberry Pi.  Falls back to plain numpy if gcc is not present.
...
Sensor stream variants:
  ...
  - FTIR    : CSV lines "engine_hours,wavenumber,absorbance[,temp,turbidity,fuel_dil]"
              FTIRModel maps absorbance bands → TBN automatically
...
INTERP_PATH      = "/home/pi/oil_health/models/interpolation.npz"   # spline fallback knots
FTIR_MODEL_PATH  = "/home/pi/oil_health/models/ftir_model.npz"      # FTIR MLR coefficients
...
FTIR_DATA_PATH   = "/home/pi/oil_health/data/ftir_samples.csv"      # FTIR spectral measurements
...
#   DERIVED_FEATURES — computed via FTIR model (primary) or spline fallback (secondary)
...
# ── FTIR spectral band definitions (Wolak et al. 2022, Table 4 / Model C) ────
# Five differential absorbance bands used as predictors for TBN
FTIR_BANDS: List[tuple] = [
    ("abs_lt1000",    650,  1000),   # fingerprint / aromatic ring region
    ("abs_1000_1450", 1000, 1450),   # sulfone additives, sulfonates
    ("abs_1475_1800", 1475, 1800),   # carbonyl, nitro, organic-oxygen products
    ("abs_1800_2830", 1800, 2830),   # baseline raise — soot/resin products
    ("abs_2975_4000", 2975, 4000),   # baseline raise — oxidation products
]
FTIR_BAND_NAMES = [b[0] for b in FTIR_BANDS]

# Excluded noise ranges (artifact from differential spectrum subtraction)
FTIR_NOISE_RANGES = [(1451, 1474), (2837, 2979)]

# ── Paper MLR coefficients — Model C (Wolak et al. 2022, Table 4) ────────────
# TBN = intercept + sum(coef_i * avg_absorbance_i) across 5 bands
# R² = 0.9815, adjusted R² = 0.9684, RMSE = 0.4050
PAPER_MLR_COEF_C: Dict[str, float] = {
    "intercept":      9.682,
    "abs_lt1000":   316.270,
    "abs_1000_1450": -10.326,
    "abs_1475_1800": -133.000,
    "abs_1800_2830":  -36.081,
    "abs_2975_4000":    3.841,
}

# Model D — peak absorbance at specific wavenumbers (R²=0.9791, RMSE=0.4313)
# Used as cross-validation / secondary estimate
PAPER_MLR_COEF_D: Dict[str, float] = {
    "intercept":   9.033,
    "abs_1746":   38.416,   # carbonyl (esters)
    "abs_1631":   14.971,   # NO₂ asymmetric bend (nitration)
    "abs_1196":  -287.984,  # sulfones / sulfonates
    "abs_1169":   82.665,   # sulfones / sulfonates
    "abs_1062":  154.500,   # S=O vibration
}

# Paper sample dataset (Table 5, Model C) — used to bootstrap & validate offline MLR
# Each row: [abs_lt1000, abs_1000_1450, abs_1475_1800, abs_1800_2830, abs_2975_4000, TBN]
PAPER_FTIR_DATASET = np.array([
    [0.0281, 0.0598, 0.0627, 0.0524, 0.0771, 8.18],
    [0.0417, 0.0960, 0.0996, 0.0800, 0.1196, 6.13],
    [0.0508, 0.1177, 0.1210, 0.1010, 0.1524, 5.24],
    [0.0629, 0.1432, 0.1450, 0.1243, 0.1870, 5.07],
    [0.0824, 0.1855, 0.1829, 0.1582, 0.2371, 4.67],
    [0.1100, 0.2400, 0.2377, 0.2102, 0.3152, 4.36],
    [0.1227, 0.2666, 0.2648, 0.2377, 0.3583, 2.85],
    [0.1361, 0.2944, 0.2932, 0.2629, 0.3979, 3.10],
    [0.1587, 0.3289, 0.3363, 0.3007, 0.4507, 2.13],
    [0.1688, 0.3497, 0.3618, 0.3196, 0.4804, 1.92],
    [0.1982, 0.3983, 0.4176, 0.3736, 0.5583, 1.67],
    [0.2138, 0.4265, 0.4489, 0.4022, 0.6004, 0.58],
    [0.2302, 0.4563, 0.4825, 0.4375, 0.6503, 0.50],
], dtype=np.float64)

# Bootstrap knots from paper (spline fallback when FTIR input is unavailable)
...
class FTIRInput:
    """
    FTIR spectral input for one measurement point.

    The sensor can provide either:
      (a) Pre-averaged band absorbances (abs_bands dict) — fastest path
      (b) Raw spectrum as {wavenumber: absorbance} pairs — averaged into bands
      (c) Scalar conditions (temp, turbidity, fuel_dilution) to estimate absorbance
          from the secondary calibration model when a spectrometer is not present.

    Only one of abs_bands / raw_spectrum / conditions needs to be provided.
    FTIRModel.estimate() tries (a), then (b), then (c), then spline fallback.
    """
    engine_hours:   float
    engine_id:      str   = "engine_1"
    # Option A: pre-averaged differential absorbance per band (fastest)
    abs_bands:      Optional[Dict[str, float]] = None
    # Option B: raw differential spectrum {wavenumber_cm1: absorbance}
    raw_spectrum:   Optional[Dict[float, float]] = None
    # Option C: physical conditions to estimate absorbance via secondary MLR
    temperature:    Optional[float] = None   # °C  — affects OH stretch / carbonyl bands
    turbidity:      Optional[float] = None   # NTU — proxy for soot / particulate load
    fuel_dilution:  Optional[float] = None   # %   — fuel-in-oil dilution fraction
    wavenumber:     Optional[float] = None   # cm⁻¹ — single-wavenumber absorbance query


@dataclass
class FTIRSample:
    """
    Logged FTIR lab measurement for offline model retraining.
    One sample = one oil extraction point with known TBN + full band absorbances.
    """
    engine_hours:   float
    tbn_measured:   float            # lab-measured TBN (mg KOH/g) — ground truth
    abs_bands:      Dict[str, float] # averaged differential absorbances per band
    temperature:    float = 25.0     # °C at measurement time
    turbidity:      float = 0.0      # NTU
    fuel_dilution:  float = 0.0      # %
    engine_id:      str   = "engine_1"
...
# Distance method decision at current scale (K=3, D=5):
#
#   Method                     | µs/call | Notes
#   ---------------------------|---------|----------------------------------
#   scipy cdist cityblock      |  ~4 µs  | ← used in Python fallback below
#   numpy abs+sum              |  ~7 µs  | array broadcast overhead dominates
#   pure Python loop           |  ~3 µs  | wins at K=3 but doesn't scale
#   Rust subprocess (pipe)     | ~500 µs | pipe round-trip swamps compute
#   Rust via ctypes FFI        |  ~2 µs  | fastest C path; needs .so build step
#
# Decision: keep Rust binary as PRIMARY path (Rust's hot-loop is <1 µs of pure
# compute, as designed in main.rs, but the pipe overhead lands at ~500 µs — still
# well inside the 1-2s tick budget).  The Python fallback uses scipy cdist which
# is consistently 1.3–1.7× faster than numpy abs+sum across all K values tested,
# requires no build step, and is already a dependency.  The ctypes C-kernel option
# would save ~2 µs on a path that's only reached when the Rust binary is absent,
# so the build complexity isn't justified at this dataset size.
class RustInferenceEngine:
    ...
    def _python_fallback(self, reading: SensorReading) -> dict:
        """
        Python fallback when Rust binary is unavailable.

        Uses scipy.spatial.distance.cdist with metric='cityblock' (L1/Manhattan).
        Benchmarked ~4 µs/call vs ~7 µs for numpy abs+sum at K=3, D=5 — scipy
        wins at every K because its C loop avoids intermediate array allocation.
        Import is deferred so the main hot path never pays the import cost.
        """
        from scipy.spatial.distance import cdist

        # Default centroids (paper values, already in MinMax-normalized space)
        # Mirrors KMeansModel::default_from_paper() in main.rs exactly
        centroids = np.array([
            [0.82, 0.09, 0.14, 0.08, 0.00],   # Healthy
            [0.55, 0.45, 0.28, 0.35, 0.40],   # Warning
            [0.10, 0.85, 0.75, 0.80, 0.90],   # Degraded
        ], dtype=np.float32)
        feat_min = np.array([5.0,  2.0, 12.0, 2.0, 0.0], dtype=np.float32)
        feat_max = np.array([11.0, 5.0, 32.0, 4.0, 5.0], dtype=np.float32)

        raw  = reading.to_array()                                   # shape (5,)
        norm = np.clip((raw - feat_min) / (feat_max - feat_min + 1e-8), 0.0, 1.0)

        # cdist expects (1, D) query — returns (1, K) distance matrix
        dists = cdist(norm.reshape(1, -1), centroids, metric="cityblock")[0]  # shape (K,)

        best         = int(np.argmin(dists))
        healthy_dist = float(dists[0])   # distance to cluster 0 (Healthy centroid)
        # Match Rust health_score formula: (1 - healthy_dist / N_FEATURES) * 100
        health_score = max(0, min(100, int((1.0 - healthy_dist / len(FEATURES)) * 100)))
